...
- ✅ Auto-publish + settle (Option C) — `php artisan draws:auto-settle` (scheduled `everyFiveMinutes()->onOneServer()->withoutOverlapping()` in `routes/console.php`). Gated by the `scraper.auto_publish_enabled` toggle (default OFF, requires admin to type `AUTO-PUBLISH` to enable). For each awaiting draw: scraper fetch → range-validate (defense in depth) → wrap `DrawResult::create` + `SettleDrawAction` in a per-draw transaction. Idempotent via `DrawAlreadySettledException`. Every auto-settle + every skip writes an `audit` log line. `--force` flag bypasses the toggle for ops debugging; `--draw=N` targets a single draw. Shared `App\Actions\Settlement\ScrapeAndSettleAwaitingAction` backs both the cron and the admin-triggered manual scrape button.
- ✅ Manual scrape button — `/admin/draws` ships a "Scrape PCSO results" button (always visible, top-right of the heading) that POSTs to `admin.draws.scrape` (`App\Http\Controllers\Admin\DrawResultController::scrape`). Same `ScrapeAndSettleAwaitingAction` as the cron, but **bypasses the `scraper.auto_publish_enabled` toggle** — an admin click is explicit consent. Flashes a summary (`Scrape done — N settled, M skipped, ₱X paid out.` or `No awaiting draws to scrape.`) and writes `admin.draws.scrape` to the `audit` channel.
- ✅ Admin auth split — `users.password` column (nullable, hashed cast); admins authenticate via `/admin/login` (username + password) at `App\Http\Controllers\Auth\AdminLoginController`, throttled `admin-login` (5/min per username+IP, 20/min per IP). Login success → redirect to `/admin` (`App\Http\Controllers\Admin\DashboardController`) with stat strip (awaiting draws, bets today, paid out today) + quick-link cards to /admin/draws, /admin/settings, /admin/wallets. Admins blocked from player surfaces (`/lotto`, `/results`, `/tickets`, `/wallet`, `/games/*`) via `EnsureAccountSetupIsComplete` — they 302 to `/admin` instead. Bootstrap a real admin password via `php artisan admin:set-password {username}` (interactive; min 6 chars, no complexity rule — matches dev/staging convenience; tighten via env-gated rule if production needs it). Player PIN flow at `/login` unchanged.
- ✅ Slot-aware draw labels — new `resources/js/lib/draw-time.ts` exposes `slotLabel()` ("2PM", "5PM", "9PM" with graceful `g:i A` fallback) and `formatDrawRow()` ("5PM - MON MAY 18"), both rendered in `Asia/Manila` regardless of the user's browser timezone. `LottoHomeController` ships a new `latest_drawn_label` prop derived from the latest settled draw's hour. `GameCard` shows "5PM RESULT" eyebrow + "5PM - MON MAY 18" draw row; `SelectDrawSheet` formats each ADVANCE row the same way; `ResultsIndex` per-row uses the compact `2PM` slot. Matches the PCSO reference site's presentation.
- ✅ Dev fixtures slimmed to users + wallets only — `DevFixturesSeeder` no longer creates fake draws / fake `DrawResult`s / fake bets. Eliminates the random `5:41 AM` / `6:30 PM` draw times that appeared whenever the seeder ran. After `db:seed`, run `php artisan draws:generate-upcoming --days=7` to populate real 14:00/17:00/21:00 Manila slots; `php artisan draws:auto-settle` (or admin publish) pulls real PCSO numbers via the scraper. `DatabaseSeeder` prints the cron hint after seeding so the next dev sees it.
- ✅ Playwright sidecar for `pcso.gov.ph` — standalone `scraper/` Node service (Express + Playwright + Chromium) exposes `GET /scrape` on `127.0.0.1:8787` returning parsed JSON, bypassing Akamai's bot-WAF that 403s plain `Http::get()` and the PH DNS block on `lottopcso.com`. Driver `App\Services\Scrapers\PcsoGovDriver` gained `pickFromRows()` to consume the JSON alongside the existing HTML `parse()`. `PcsoResultScraper` routes between Guzzle and the sidecar via `LOTTO_SCRAPER_FETCHER=http|playwright` (default `http`, source `lottopcso`). Sidecar is managed as a Forge daemon (see `scraper/README.md`). Cache key `lotto.scraper:sidecar_rows` means one fetch per 60s covers every awaiting draw in the loop. **Note:** vanilla headless Chromium still fingerprinted by Akamai — needs `playwright-extra` + stealth plugin to actually punch through, which we deferred in favour of the GMA Network source below.
- ✅ GMA Network scraper source — `App\Services\Scrapers\GmaNetworkDriver` reads `https://www.gmanetwork.com/news/lotto/` (plain `Http::get()`, no WAF, no DNS block, reachable from PH ISPs). Matches by `data-type` + `data-date` attributes on each result anchor. GMA labels 3D as "Swertres" (not "3D Lotto"); driver normalizes. Live-tested against gmanetwork.com: every 2D/3D row across the listing returns the expected `list<int>`. Recommended default — flip `LOTTO_SCRAPER_SOURCE=gma` in production. `lottopcso` + `pcso_gov` remain available as fallbacks.
- ✅ pcso-parser API integration — new `pcso_api` source/fetcher pair (`LOTTO_SCRAPER_SOURCE=pcso_api`, `LOTTO_SCRAPER_FETCHER=pcso_api`) consumes the standalone Node REST API at `http://127.0.0.1:3001/fetch` (separate repo/daemon, returns parsed JSON). `App\Services\Scrapers\PcsoParserApiDriver::pickFromRows()` looks up the matching row by stable `_id` reconstruction (`YYYY-MM-DD-{2D|3D}-{200|500|900}PM`) — no fuzzy string match. `App\Services\Scrapers\PcsoParserApiClient` handles the HTTP call with one 429 retry. Cache key `lotto.scraper:pcso_api_rows:{date}` means one API call covers every awaiting draw on that date for 60s. Both the admin manual button and `draws:auto-settle` cron use it automatically — zero caller changes. Forge: run the API as a daemon bound to 127.0.0.1:3001; the box's egress must be able to reach `lottopcso.com`.
- ✅ Backfill historical results — `App\Actions\Settlement\BackfillDrawResultsAction` + `php artisan draws:backfill-results [--from --to | --days=N] [--games=2d,3d] [--dry-run]` + `POST /admin/draws/backfill` (button next to "Scrape PCSO results", defaults to last 7 days). Upserts `DrawResult` for past draws in a date range using the configured scraper. **Never settles bets** (sibling to scrape, not a replacement): no `SettleDrawAction` call, `Draw.status` untouched, `Bet.status` untouched, no wallet credits. Refuses to overwrite numbers on already-settled draws (would silently desync paid bets — operator must reverse the ledger manually first). Audit-logs every per-draw outcome (`draw.backfill.created` / `updated` / `unchanged` / `skipped_settled` / `scraper_returned_null`).
